...
export const XMRigView:React.FC<XMRigViewProps> = ({
  ...
  const RenderCPUGrid = React.useCallback(() => (
    <GridView
      items={[
        {
          renderCustomItem: () => (
            <GridCard title="品牌" text={minerData?.cpu.brand || 'N/A'}>
        ...
        {
          renderCustomItem: () => (
            <GridCard title="核心" text={minerData?.cpu.cores || 'N/A'}>
        ...
        { renderCustomItem: () => <GridCard title="线程" text={minerData?.cpu.threads || 'N/A'} /> },
  ...
  const RenderSharesGrid = React.useCallback(() => (
    <GridView
      items={[
        { renderCustomItem: () => <GridCard title="通过" text={minerData?.connection.accepted || 0} /> },
        { renderCustomItem: () => <GridCard title="被拒绝" text={minerData?.connection.rejected || 0} /> },
        { renderCustomItem: () => <GridCard title="总计" text={(minerData?.connection.accepted || 0) + (minerData?.connection.rejected || 0)} /> },
      ]}
      numColumns={3}
      viewWidth={dimensions.width}
    />
  ...
  const RenderSharesMoreGrid = React.useCallback(() => (
    <GridView
      items={[
        { renderCustomItem: () => <GridCard title="困难" text={minerData?.results.diff_current || 'N/A'} /> },
        { renderCustomItem: () => <GridCard title="总计哈希" text={hashrateToString(minerData?.results.hashes_total || 0, true)} /> },
      ]}
      numColumns={2}
      viewWidth={dimensions.width}
    />
  ...
  const RenderMemoryGrid = React.useCallback(() => (
    <GridView
      items={[
        {
          renderCustomItem: () => (
            <GridCard title="空闲内存" text={prettyBytes(minerData?.resources.memory.free || 0)}>
        ...
        {
          renderCustomItem: () => (
            <GridCard title="资源内存" text={prettyBytes(minerData?.resources.memory.resident_set_memory || 0)}>
              <Card.Image source={Assets.icons.memory} height={25} width={28} style={{ position: 'absolute', right: 10, top: 10 }} tintColor={Colors.$iconNeutral} />
            </GridCard>
          ),
        },
  ...
  const RenderModeAlgoGrid = React.useCallback(() => (
    <GridView
      items={[
        ...
        {
          renderCustomItem: () => (
            <GridCard title="算法" text={minerData?.algo || 'N/A'}>
